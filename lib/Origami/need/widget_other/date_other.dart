...
class DateOrder extends StatefulWidget {
  const DateOrder({
    super.key, required this.firstDay, required this.getfirstDay,
  });
  final String getfirstDay;
  final String Function(String) firstDay;
  ...
  @override
  State<DateOrder> createState() => DateOrderState();
}

class DateOrderState extends State<DateOrder> {
  String showfirstDay = '';
  ...
  DateTime _selectedDate = DateTime.now();
  Future<void> _itemDate(BuildContext context) async {
    await showModalBottomSheet(
      context: context,
      builder: (BuildContext context) {
        return Theme(
  ...
  Widget build(BuildContext context) {
    return Row(
      ...
      children: [
        Text(
          '$Date : ',
          style: GoogleFonts.openSans(fontSize: 16, fontWeight: FontWeight.bold,color: Color(0xFF555555)),
        ),
        Expanded(
          child: Padding(
            padding: const EdgeInsets.only(
