...
import 'package:flutter_project/Origami/need/widget_mini/mini_department.dart';
import 'package:flutter_project/Origami/need/widget_mini/mini_employee.dart';
import 'package:flutter_project/Origami/need/widget_mini/mini_project.dart';
import 'package:flutter_project/Origami/need/widget_other/date_other.dart';
import 'package:flutter_project/Origami/need/widget_other/priority_other.dart';
...
class VerticalCardPagerPage extends StatelessWidget {
  final List<File> images;
  final int initialIndex;

  VerticalCardPagerPage({required this.images, required this.initialIndex});

  @override
  Widget build(BuildContext context) {
    final List<Widget> imageWidgets = images.map((file) {
      return Container(
        decoration: BoxDecoration(
          borderRadius: BorderRadius.circular(15),
          border: Border.all(
        ...
        children: [
          Center(
            child: VerticalCardPager(
              ...
              titles: images.map((e) => "").toList(),
              images: imageWidgets,
              onPageChanged: (page) {
                // สามารถใส่โค้ดเมื่อมีการเปลี่ยนหน้าได้ที่นี่
              },
              onSelectedItem: (index) {
                // สามารถใส่โค้ดเมื่อเลือกภาพได้ที่นี่
              },
            ),
          ),
          Positioned(
...
class TrianglePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    var path = Path();
    path.moveTo(size.width / 2, size.height * 0.5); // จุดบนสุดของสามเหลี่ยม
    path.lineTo(size.width, size.height); // มุมขวาล่างของสามเหลี่ยม
    path.lineTo(0, size.height); // มุมซ้ายล่างของสามเหลี่ยม
    path.close();
...
class TrianglePainter2 extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    var path = Path();
    path.moveTo(size.width / 2, size.height * 0.9); // จุดบนสุดของสามเหลี่ยม
    path.lineTo(size.width, size.height); // มุมขวาล่างของสามเหลี่ยม
    path.lineTo(0, size.height); // มุมซ้ายล่างของสามเหลี่ยม
    path.close();
